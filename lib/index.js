...
function createTree(comparer) {
  ...
  function validateValues(values) {
    if (!Array.isArray(values)) return Promise.reject(new BbTreeError('Array expected'));
  ...
  function insertValues(values) {
    ...
    values.forEach(value => {
      result = result
        ...
        .then(insertedValue => {
          if (insertedValue !== null) insertedValues.push(insertedValue);
          return Promise.resolve();
        });
    });
    return result.then(() => { return Promise.resolve(insertedValues); });
  }
  ...
  return {
    ...
    get root() { return root ? root.value : root; },
    BbTreeError: () => { return BbTreeError; }
  };
}
