...
function createTree(comparer) {
  ...
  function insert(node, value) {
    ...
    if (balance > 1 && (cmp(value, node.left.value) === -1)) {
      return rightRotate(node);
    }
    if (balance < -1 && (cmp(value, node.right.value) === 1)) {
  ...
  function get(node, value) {
    ...
    if (cmpResult === -1) return get(node.left, value);
  ...
  return {
    ...
    delete: (value) => { root = deleteNode(root, value); },
